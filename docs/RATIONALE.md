...
```bash
step "API Spec Drift Check"
bun run spec:export
if ! git diff --quiet -- openapi.json; then
  echo "::error::spec changed but was not committed — run spec:export"
  failed=$((failed + 1))
fi
```

Without it the contract degrades quietly, and the cost lands in someone else's repo.
...
document is **much smaller**. Security and data-access rules have to be spelled out — "validate input at system
